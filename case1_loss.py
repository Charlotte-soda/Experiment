...
# data = getData()
readvocab = torch.load('vocab_np')     # 读取vocab文件
# print(readvocab)
...
# print(readvocab[130], readvocab[963])
...
def case1_loss1(input1, input2, out1, out2):
    ...
    eps = 1e-8
    loss1 = torch.sqrt(torch.pow((input1*input2 - out1*out2),2) + torch.pow((input1**2 * input2**2)/2 - (out1**2 * out2**2)/2, 2) + eps)
